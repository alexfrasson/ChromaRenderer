...
bool RTUtils::closestIntersectionRayObjects(const Ray& ray,
                                            ...
                                            const bool boundingboxtest)
{
    ...
    const size_t vectorSize = objects.size();
...
bool RTUtils::intersectUntilRayObjects(const Ray& ray,
                                       const std::vector<Object>& objects,
...
bool RTUtils::intersectRayTrianglesMollerTrumboreSIMD128(const Ray& r,
                                                         ...
                                                         float& hitDistance)
{
    ...
    for (size_t i = sequentialsize; i + 3 < f.size(); i += 4)
    {
        ...
        const __m128 pvecx = _mm_sub_ps(_mm_mul_ps(dy, edge1z), _mm_mul_ps(dz, edge1y));
        const __m128 pvecy = _mm_sub_ps(_mm_mul_ps(dz, edge1x), _mm_mul_ps(dx, edge1z));
        const __m128 pvecz = _mm_sub_ps(_mm_mul_ps(dx, edge1y), _mm_mul_ps(dy, edge1x));
        // Compute det = dot(edge0, pvec)
        const __m128 det =
            _mm_add_ps(_mm_add_ps(_mm_mul_ps(pvecx, edge0x), _mm_mul_ps(pvecy, edge0y)), _mm_mul_ps(pvecz, edge0z));
        // If determinant is near zero, ray lies in plane of triangle
        ...
        if (_mm_movemask_ps(mask) == 0x0)
            continue;
        //__m128 mask = _mm_or_ps(detmasklt, detmaskgt);			//without backface culling (if(det < -EPSILON || det
        //> EPSILON)) const __m128 rcp = _mm_rcp_ps(det);								//Reciprocal inveDev = 1.0/dev7
        ...
        const __m128 p0x = _mm_set_ps(f[i].v[0].x, f[i + 1].v[0].x, f[i + 2].v[0].x, f[i + 3].v[0].x);
        const __m128 p0y = _mm_set_ps(f[i].v[0].y, f[i + 1].v[0].y, f[i + 2].v[0].y, f[i + 3].v[0].y);
        const __m128 p0z = _mm_set_ps(f[i].v[0].z, f[i + 1].v[0].z, f[i + 2].v[0].z, f[i + 3].v[0].z);
        ...
        const __m128 u = _mm_mul_ps(
            _mm_add_ps(_mm_add_ps(_mm_mul_ps(pvecx, tvecx), _mm_mul_ps(pvecy, tvecy)), _mm_mul_ps(pvecz, tvecz)),
            invDet);
        // Test u >= 0.0 && u <= 1.0
        ...
        const __m128 qvecx = _mm_sub_ps(_mm_mul_ps(tvecy, edge0z), _mm_mul_ps(tvecz, edge0y));
        const __m128 qvecy = _mm_sub_ps(_mm_mul_ps(tvecz, edge0x), _mm_mul_ps(tvecx, edge0z));
        const __m128 qvecz = _mm_sub_ps(_mm_mul_ps(tvecx, edge0y), _mm_mul_ps(tvecy, edge0x));
        // Compute v = dot(r.direction, qvec) * invDet;
        const __m128 v =
            _mm_mul_ps(_mm_add_ps(_mm_add_ps(_mm_mul_ps(dx, qvecx), _mm_mul_ps(dy, qvecy)), _mm_mul_ps(dz, qvecz)),
                       invDet);
        // Test v >= 0.0 && v <= 1.0
        ...
        const __m128 t = _mm_mul_ps(
            _mm_add_ps(_mm_add_ps(_mm_mul_ps(qvecx, edge1x), _mm_mul_ps(qvecy, edge1y)), _mm_mul_ps(qvecz, edge1z)),
            invDet);
        // Test t > EPSILON
...
bool RTUtils::intersectRayObjectMollerTrumboreSIMD128(const Ray& r,
                                                      const Object& o,
                                                      const Face** hitFace,
                                                      float& hitDistance)
{
    ...
    for (size_t i = 0; i + 3 < o.f.size(); i += 4)
    {
        ...
        // const __m128 invDet = _mm_rcp_ps(det);
        // Load vertice 0 of each triangle
        const __m128 p0x = _mm_set_ps(o.f[i].v[0].x, o.f[i + 1].v[0].x, o.f[i + 2].v[0].x, o.f[i + 3].v[0].x);
        const __m128 p0y = _mm_set_ps(o.f[i].v[0].y, o.f[i + 1].v[0].y, o.f[i + 2].v[0].y, o.f[i + 3].v[0].y);
        const __m128 p0z = _mm_set_ps(o.f[i].v[0].z, o.f[i + 1].v[0].z, o.f[i + 2].v[0].z, o.f[i + 3].v[0].z);
        // Compute tvec = rayorigin - vertice0
        const __m128 tvecx = _mm_sub_ps(ox, p0x);
        const __m128 tvecy = _mm_sub_ps(oy, p0y);
        const __m128 tvecz = _mm_sub_ps(oz, p0z);
...
bool RTUtils::intersectRayObjectMollerTrumboreSIMD256(const Ray& r,
                                                      const Object& o,
                                                      const Face** hitFace,
                                                      float& hitDistance)
{
    ...
    for (size_t i = 0; i + 7 < o.f.size(); i += 8)
    {
        ...
        const __m256 index =
            _mm256_set_ps(fi, fi + 1.0f, fi + 2.0f, fi + 3.0f, fi + 4.0f, fi + 5.0f, fi + 6.0f, fi + 7.0f); // WHY????
        // Load edge 0 of each triangle
        ...
        // Compute pvec = cross(raydir, edge1)
        const __m256 pvecx = _mm256_sub_ps(_mm256_mul_ps(dy, edge1z), _mm256_mul_ps(dz, edge1y));
        const __m256 pvecy = _mm256_sub_ps(_mm256_mul_ps(dz, edge1x), _mm256_mul_ps(dx, edge1z));
        const __m256 pvecz = _mm256_sub_ps(_mm256_mul_ps(dx, edge1y), _mm256_mul_ps(dy, edge1x));
        // Compute det = dot(edge0, pvec)
        const __m256 det = _mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(pvecx, edge0x), _mm256_mul_ps(pvecy, edge0y)),
        ...
        // Compute u = dot(tvec, pvec) * invDet;
        const __m256 u =
            _mm256_mul_ps(_mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(pvecx, tvecx), _mm256_mul_ps(pvecy, tvecy)),
                                        _mm256_mul_ps(pvecz, tvecz)),
                          invDet);
        // Test u >= 0.0 && u <= 1.0
        ...
        if (_mm256_movemask_ps(mask) == 0x0)
            continue;
        // Compute qvec = cross(tvec, edge0)
        const __m256 qvecx = _mm256_sub_ps(_mm256_mul_ps(tvecy, edge0z), _mm256_mul_ps(tvecz, edge0y));
        const __m256 qvecy = _mm256_sub_ps(_mm256_mul_ps(tvecz, edge0x), _mm256_mul_ps(tvecx, edge0z));
        const __m256 qvecz = _mm256_sub_ps(_mm256_mul_ps(tvecx, edge0y), _mm256_mul_ps(tvecy, edge0x));
        // Compute v = dot(r.direction, qvec) * invDet;
        const __m256 v = _mm256_mul_ps(
            _mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(dx, qvecx), _mm256_mul_ps(dy, qvecy)), _mm256_mul_ps(dz, qvecz)),
            invDet);
        // Test v >= 0.0 && v <= 1.0
        ...
        // Compute t = dot(edge1, qvec) * invDet;
        const __m256 t =
            _mm256_mul_ps(_mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(qvecx, edge1x), _mm256_mul_ps(qvecy, edge1y)),
                                        _mm256_mul_ps(qvecz, edge1z)),
                          invDet);
        // Test t > EPSILON
