...
ThreadPool::ThreadPool() : quit(false)
{
...
ThreadPool::ThreadPool(unsigned int maxNumberWorkers) : quit(false)
{
...
// If 'maxNumberWorkers' is 0, the capacity is set to the number of processors.
ThreadPool::ThreadPool(unsigned int maxNumberWorkers, size_t nWorkers) : quit(false)
{
    capacity = maxNumberWorkers;
    if (capacity < 1)
    {
                                                ...
                                                for (;;)
                                                {
                                                    std::function<void(bool&)> task;
                                                    {
                                                        ...
                                                        this->condition.wait(lock, [this, i] {
                                                            return this->quit || QUIT(this->workers[i]) ||
                                                                   !this->tasks.empty();
                                                        });
