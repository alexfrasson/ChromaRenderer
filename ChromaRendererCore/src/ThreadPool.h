...
private:
	...
	// If the ThreadPool needs to close one (or all) of the workers, it will not have to wait the worker to complete the task.
	// E.g.
	// void task(bool& stop)
	// {
	...
	// The first boolean is used to stop the current task. After the current taks is stopped, the worker will set this boolean to false again and search for another task.
	// The second boolean is used to completely close the worker. It will be delete from the worker list after it finishes.
