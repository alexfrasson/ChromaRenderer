...
__device__ __inline__ float3 cosineSampleHemisphere(curandState *randState, float3 normal)
{
	...
	float3 u = cross((fabs(w.x) > .1 ? make_float3(0, 1, 0) : make_float3(1, 0, 0)), w); u = normalize(u);
	float3 v = cross(w, u);
...
__device__ __inline__ CudaRay rayDirectionWithOffset(const int i, const int j, CudaCamera cam, curandState *randState)
{
	...
	ray.direction = normalize((float)(i + random0 - cam.width*0.5f) * cam.right + (float)(j + random1 - cam.height*0.5f) * cam.up + cam.d * cam.forward);
	return ray;
}

// Computes ray direction given camera object and pixel position
extern "C"
__host__ __device__ __inline__ CudaRay rayDirection(const int i, const int j, CudaCamera cam)
{
	CudaRay ray;
	ray.mint = 0;
	ray.maxt = FLT_MAX;
	ray.origin = cam.eye;
	ray.direction = (float)(i - cam.width / 2.0f) * cam.right + (float)(j - cam.height / 2.0f) * cam.up + cam.d * cam.forward;
...
__host__ __device__ __inline__ bool hitBoundingBoxSlab(const CudaBoundingBox& bb, const CudaRay& r, const float3& invRayDir, const bool* dirIsNeg, float& tmin, float& tmax)
{
...
__host__ __device__ bool intersectBVH(const CudaTriangle *triangles, const CudaLinearBvhNode* linearBVH, CudaRay& r, CudaIntersection& intersection)
{
	bool hit = false;
...
__global__ void traceKernel(CudaPathIteration* pathIterationBuffer, float4* accuBuffer, const dim3 texDim, const CudaCamera cam, const CudaTriangle *triangles, int nTriangles, const CudaMaterial *materials, const unsigned int nMaterials, const unsigned int seed, const CudaLinearBvhNode* linearBVH, const CudaEnviromentSettings enviromentSettings)
{
	...
	if (!intersectBVH(triangles, linearBVH, ray, is))
	{
		...
		//float4 env = tex2D<float4>(enviromentSettings.texObj, u, 1.0f - v) * enviromentSettings.enviromentLightIntensity;
		//color = pathIteration.mask * make_float3(pow(env.x, 1.0f / 2.2f), pow(env.y, 1.0f / 2.2f), pow(env.z, 1.0f / 2.2f));
	...
	else
	{
		...
		if (materials[is.material].transparent.x < 1.0 || materials[is.material].transparent.y < 1.0 || materials[is.material].transparent.z < 1.0)
		{
...
void bindTextureToArray(cudaArray* aarray)
{
	...
	cudaChannelFormatDesc channelDesc; checkCudaErrors(cudaGetChannelDesc(&channelDesc, aarray));
...
void trace(cudaStream_t &stream, CudaPathIteration* pathIterationBuffer, float4* accuBuffer, dim3 texDim, CudaCamera cam, CudaTriangle* triangles, unsigned int nTriangles, CudaMaterial* materials, unsigned int nMaterials, unsigned int seed, CudaLinearBvhNode* linearBVH, CudaEnviromentSettings enviromentSettings)
{
	...
	traceKernel<<< block, thread, 0, stream >>>(pathIterationBuffer, accuBuffer, texDim, cam, triangles, nTriangles, materials, nMaterials, seed, linearBVH, enviromentSettings);
}

#ifdef DEPRECATED_CODE
// Deprecated
__device__ float3 tracePath_deprecated(curandState *randState, CudaRay *ray, const CudaTriangle *triangles, const int nTriangles, const CudaMaterial *materials, const unsigned int nMaterials, const CudaLinearBvhNode* linearBVH, const CudaEnviromentSettings enviromentSettings)
{
...
__global__ void raycastKernel_deprecated(cudaSurfaceObject_t s, dim3 texDim, CudaCamera cam, CudaTriangle *triangles, unsigned int nTriangles, CudaMaterial *materials, unsigned int nMaterials)
{
	const unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
	const unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
	if (x >= texDim.x || y >= texDim.y)
...
__global__ void raycastKernel_deprecated(cudaSurfaceObject_t s, dim3 texDim, CudaCamera cam, CudaTriangle *triangles, unsigned int nTriangles, CudaMaterial *materials, unsigned int nMaterials, CudaLinearBvhNode* linearBVH)
{
	const unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
	const unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
	if (x >= texDim.x || y >= texDim.y)
...
// Traces rays into the scene intersecting each one with all triangles.
//__global__ void traceKernel(cudaSurfaceObject_t s, dim3 texDim, CudaCamera cam, CudaTriangle *triangles, int nTriangles, CudaMaterial *materials, unsigned int nMaterials, unsigned int seed)
//{
//	const unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
//	const unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
//	if (x >= texDim.x || y >= texDim.y)
...
//	color /= SAMPLES;
//
//	//data = make_uchar4(max(0.0f, min(1.0f, kd.x)) * 255, max(0.0f, min(1.0f, kd.y)) * 255, max(0.0f, min(1.0f, kd.z)) * 255, 0xff);
//	// Write back to texture
//	surf2Dwrite(make_uchar4(max(0.0f, min(1.0f, color.x)) * 255, max(0.0f, min(1.0f, color.y)) * 255, max(0.0f, min(1.0f, color.z)) * 255, 0xff), s, x * sizeof(uchar4), y);
//}
//__global__ void traceKernel(cudaSurfaceObject_t s, dim3 texDim, CudaCamera cam, CudaTriangle *triangles, int nTriangles, CudaMaterial *materials, unsigned int nMaterials, unsigned int seed, CudaLinearBvhNode* linearBVH, bool enviromentLight)
//{
//	const unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
//	const unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
//	if (x >= texDim.x || y >= texDim.y)
...
//	surf2Dwrite(make_uchar4(max(0.0f, min(1.0f, color.x)) * 255, max(0.0f, min(1.0f, color.y)) * 255, max(0.0f, min(1.0f, color.z)) * 255, 0xff), s, x * sizeof(uchar4), y);
//}
__global__ void traceKernel_deprecated(float4* accuBuffer, const dim3 texDim, const CudaCamera cam, const CudaTriangle *triangles, int nTriangles, const CudaMaterial *materials, const unsigned int nMaterials, const unsigned int seed, const CudaLinearBvhNode* linearBVH, const CudaEnviromentSettings enviromentSettings)
{
	const unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
	const unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
	if (x >= texDim.x || y >= texDim.y)
	...
	for (int i = 0; i < SAMPLES; i++)
	{
		...
		CudaRay ray = rayDirection(&randState, x, y, cam);
		// Trace path
		color += tracePath_deprecated(&randState, &ray, triangles, nTriangles, materials, nMaterials, linearBVH, enviromentSettings);
	}
...
//__device__ float3 tracePath(curandState *randState, CudaRay *ray, CudaTriangle *triangles, int nTriangles, CudaMaterial *materials, unsigned int nMaterials)
//{
//	float3 accu = make_float3(0.0f);
//	float3 mask = make_float3(1.0f);
//
...
//}
//void trace(cudaSurfaceObject_t surface, dim3 texDim, CudaCamera cam, CudaTriangle* triangles, unsigned int nTriangles, CudaMaterial* materials, unsigned int nMaterials, unsigned int seed, CudaLinearBvhNode* linearBVH, bool enviromentLight)
//{
//	dim3 thread(THREAD_DIM, THREAD_DIM);
//	dim3 block(ceilf((float)texDim.x / (float)thread.x), ceilf((float)texDim.y / (float)thread.y));
//	traceKernel << < block, thread >> >(surface, texDim, cam, triangles, nTriangles, materials, nMaterials, seed, linearBVH, enviromentLight);
//	//raycastKernel << < block, thread >> >(surface, texDim, cam, triangles, nTriangles, materials, nMaterials, linearBVH);
//}
...
//	dim3 thread(THREAD_DIM, THREAD_DIM);
//	dim3 block(ceilf((float)texDim.x / (float)thread.x), ceilf((float)texDim.y / (float)thread.y));
//	traceKernel << < block, thread >> >(accuBuffer, texDim, cam, triangles, nTriangles, materials, nMaterials, seed, linearBVH, enviromentLight);
//	//raycastKernel << < block, thread >> >(accuBuffer, texDim, cam, triangles, nTriangles, materials, nMaterials, linearBVH);
//}
