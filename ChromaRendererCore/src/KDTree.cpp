...
bool intersectLineSegmentPlaneD(const glm::dvec3& planeP, const glm::dvec3& planeN, const glm::dvec3& p0, const glm::dvec3& p1, glm::dvec3& intersection);
	...
	: root(NULL)
{
	...
	std::cout << "    Max " << "(" << clipped.max.x << ", " << clipped.max.y << ", " << clipped.max.z << ")" << std::endl
	...
	for (int i = 0; i < nBBs; i++)
	{
		for (int j = 0; j < nTris; j++)
		{
			...
			{
				//file << "Max " << glm::to_string(clipped0.max) << " Min " << glm::to_string(clipped0.min) << std::endl;
				//std::cout << "Max " << glm::to_string(clipped0.max) << std::endl << "Min " << glm::to_string(clipped0.min) << std::endl;
			}
			else
			{
				intersection = false;
			...
			if (clipTriangleToBoxSHD(tris[j], bbs[i], clipped1))
			{
				if (!intersection)
				{
					...
					std::cout << "    Max " << "(" << bbs[i].max.x << ", " << bbs[i].max.y << ", " << bbs[i].max.z << ")" << std::endl << "    Min " << "(" << bbs[i].min.x << ", " << bbs[i].min.y << ", " << bbs[i].min.z << ")" << std::endl;
					std::cout << "--Triangle:" << std::endl;
					std::cout << "    V0 " << "(" << tris[j].v[0].x << ", " << tris[j].v[0].y << ", " << tris[j].v[0].z << ")" << std::endl;
					std::cout << "    V1 " << "(" << tris[j].v[1].x << ", " << tris[j].v[1].y << ", " << tris[j].v[1].z << ")" << std::endl;
					std::cout << "    V2 " << "(" << tris[j].v[2].x << ", " << tris[j].v[2].y << ", " << tris[j].v[2].z << ")" << std::endl;
					std::cout << "--clipTriangleToBox found:" << std::endl;
					std::cout << "    No intersection" << std::endl;
					std::cout << "--clipTriangleToBoxSH found:" << std::endl;
					std::cout << "    Max " << "(" << clipped1.max.x << ", " << clipped1.max.y << ", " << clipped1.max.z << ")" << std::endl << "    Min " << "(" << clipped1.min.x << ", " << clipped1.min.y << ", " << clipped1.min.z << ")" << std::endl;

				}
				else if (clipped0.max.x == clipped1.max.x && clipped0.max.y == clipped1.max.y && clipped0.max.z == clipped1.max.z
					&& clipped0.min.x == clipped1.min.x && clipped0.min.y == clipped1.min.y && clipped0.min.z == clipped1.min.z)
				...
				else
				{
					...
					std::cout << "    V2 " << "(" << tris[j].v[2].x << ", " << tris[j].v[2].y << ", " << tris[j].v[2].z << ")" << std::endl;
					std::cout << "--clipTriangleToBox found:" << std::endl;
					std::cout << "    Max " << "(" << clipped0.max.x << ", " << clipped0.max.y << ", " << clipped0.max.z << ")" << std::endl << "    Min " << "(" << clipped0.min.x << ", " << clipped0.min.y << ", " << clipped0.min.z << ")" << std::endl;
					std::cout << "--clipTriangleToBoxSH found:" << std::endl;
					std::cout << "    Max " << "(" << clipped1.max.x << ", " << clipped1.max.y << ", " << clipped1.max.z << ")" << std::endl << "    Min " << "(" << clipped1.min.x << ", " << clipped1.min.y << ", " << clipped1.min.z << ")" << std::endl;
					std::cout << std::endl;
				}
...
KDTNode* KDTree::presortedBuildNodeSah(int depth, std::vector<TFpointers>& triandflag, std::vector<PEvent>& events, BoundingBox& nodebb)
{
	if (stop)
		return NULL;
...
void KDTree::presortedGenEvents(const std::vector<TFpointers>& tfs, const BoundingBox& nodebb, std::vector<PEvent>& events)
{
...
float KDTree::presortedEventFindBestPlane(const int triangles, const std::vector<PEvent>& events, const BoundingBox& nodebb, Side& side, Plane& plane)
{
	...
	while (i < esize)
	{
		...
		while ((i < esize) && (events[i].plane.dim == p.dim) && (events[i].plane.value == p.value) && (events[i].type == EVENT_TYPE_END))
		{
			i++;
			pend++;
		}
		while ((i < esize) && (events[i].plane.dim == p.dim) && (events[i].plane.value == p.value) && (events[i].type == EVENT_TYPE_PLANAR))
		{
			i++;
			pplanar++;
		}
		while ((i < esize) && (events[i].plane.dim == p.dim) && (events[i].plane.value == p.value) && (events[i].type == EVENT_TYPE_START))
		{
			i++;
			pstart++;
		}
	...
	{
	if (events[i].type == EVENT_TYPE_END && events[i].plane.dim == minCostPlane.dim && events[i].plane.value < minCostPlane.value)
	{
	// left only
	elo.push_back(events[i]);
	}
	else if (events[i].type == EVENT_TYPE_START && events[i].plane.dim == minCostPlane.dim && events[i].plane.value > minCostPlane.value)
	{
	// right only
	...
	{
	if (events[i].plane.value < minCostPlane.value || (events[i].plane.value == minCostPlane.value && minCostSide == Side::LEFT))
	{
	//left only
	elo.push_back(events[i]);
	}
	else if (events[i].plane.value > minCostPlane.value || (events[i].plane.value == minCostPlane.value && minCostSide == Side::RIGHT))
	{
	//right only
	ero.push_back(events[i]);
	}
...
void KDTree::presortedClassify(const std::vector<TFpointers>& tfs, const std::vector<PEvent>& events, const Plane& splitPlane, const Side& splitPlaneSide)
{
	...
	for (int i = 0; i < events.size(); i++)
	{
		if (events[i].type == EVENT_TYPE_END && events[i].plane.dim == splitPlane.dim && events[i].plane.value < splitPlane.value)
		{
			// left only
			*events[i].flag = Side::LEFT;
		}
		else if (events[i].type == EVENT_TYPE_START && events[i].plane.dim == splitPlane.dim && events[i].plane.value > splitPlane.value)
		{
			// right only
		...
		{
			if (events[i].plane.value < splitPlane.value || (events[i].plane.value == splitPlane.value && splitPlaneSide == Side::LEFT))
			{
				//left only
				*events[i].flag = Side::LEFT;
			}
			else if (events[i].plane.value > splitPlane.value || (events[i].plane.value == splitPlane.value && splitPlaneSide == Side::RIGHT))
			{
				//right only
				*events[i].flag = Side::RIGHT;
			}
...
void KDTree::presortedSplitEvents(const std::vector<PEvent>& events, const std::vector<TFpointers>& tfs, const BoundingBox& nodebb, const Plane& splitplane, std::vector<PEvent>& eventsl, std::vector<PEvent>& eventsr)
{
		...
		if (*tfs[i].flag == Side::BOTH)
		{
			...
			if (clipTriangleToBoxSHD(*tfs[i].triangle, vl, clippedl))
			{
				...
				for (int dim = 0; dim < 3; dim++)
				{
					...
					{
						bothleft.emplace_back(Plane(dim, clippedl.min[dim]), tfs[i].triangle, tfs[i].flag, EVENT_TYPE_PLANAR);
					}
					else
					{
						bothleft.emplace_back(Plane(dim, clippedl.min[dim]), tfs[i].triangle, tfs[i].flag, EVENT_TYPE_START);
						bothleft.emplace_back(Plane(dim, clippedl.max[dim]), tfs[i].triangle, tfs[i].flag, EVENT_TYPE_END);
					}
				}
			}
			...
			if (clipTriangleToBoxSHD(*tfs[i].triangle, vr, clippedr))
			{
				for (int dim = 0; dim < 3; dim++)
				{
					...
					{
						bothright.emplace_back(Plane(dim, clippedr.min[dim]), tfs[i].triangle, tfs[i].flag, EVENT_TYPE_PLANAR);
					}
					else
					{
						bothright.emplace_back(Plane(dim, clippedr.min[dim]), tfs[i].triangle, tfs[i].flag, EVENT_TYPE_START);
						bothright.emplace_back(Plane(dim, clippedr.max[dim]), tfs[i].triangle, tfs[i].flag, EVENT_TYPE_END);
					}
				}
			}
		}
	...
	std::merge(leftonly.begin(), leftonly.end(), bothleft.begin(), bothleft.end(), std::back_inserter(eventsl), peventCmp);

	eventsr.reserve(rightonly.size() + bothright.size());
	std::merge(rightonly.begin(), rightonly.end(), bothright.begin(), bothright.end(), std::back_inserter(eventsr), peventCmp);
}
bool clipTriangleToBoxSHD(const Face& t, const BoundingBox& v, BoundingBox& clipped)
{
		...
		for (int j = 0; j < vertices.size(); j++)
		{
			...
			if (vertices[j][dim] < v.min[dim])
			{
				...
				else if (vertices[next][dim] > v.min[dim])
				{
					...
					if (intersectLineSegmentPlaneD(glm::dvec3(v.min), planeN, vertices[j], vertices[next], intersection))
					{
		...
		for (int j = 0; j < vertices.size(); j++)
		{
			...
			if (vertices[j][dim] > v.max[dim])
			{
				...
				else if (vertices[next][dim] < v.max[dim])
				{
					...
					planeN[dim] = 1.f;
					// Add the intersection point and vertices[next]
					if (intersectLineSegmentPlaneD(glm::dvec3(v.max), planeN, vertices[j], vertices[next], intersection))
					{
						newvertices.emplace_back(intersection);
						newvertices.emplace_back(vertices[next]);
					}
...
float KDTree::cost(const float probabilityL, const float probabilityR, const int triangleCountL, const int triangleCountR)
{
	float bonus = 1.f;
...
void KDTree::printInfo()
{
	std::cout << "Node #"
		...
		<< "Max. depth:      " << depth
		<< std::endl
		<< "Tri. references: " << nTriangles
		<< std::endl
		<< "Avg. tris/leaf:  " << (float)nTriangles / (float)nLeafs
		<< std::endl
...
bool intersectLineSegmentPlane(const glm::vec3& planeP, const glm::vec3& planeN, const glm::vec3& p0, const glm::vec3& p1, glm::vec3& intersection)
{
		...
		if (left.size() != 3 && right.size() != 3)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				if (t >= 0.f && t <= 1.f)
				{
					...
					int dim0 = (dim + 1) % 3;
					int dim1 = (dim + 2) % 3;
					if (p[dim0] >= v.min[dim0] && p[dim0] <= v.max[dim0] && p[dim1] >= v.min[dim1] && p[dim1] <= v.max[dim1])
					{
...
void KDTree::classify(const std::vector<std::pair<BoundingBox, int>>& ts, const Plane p, std::vector<int>& tl, std::vector<int>& tr, std::vector<int>& tp)
{
	// Number fo triangles to the left, to the right and the number of triangles that lie on the plane
	tl.clear();
	tr.clear();
...
bool KDTree::naiveSahPartition(const std::vector<Face>& ts, BoundingBox nodebb, Plane& splitP, std::vector<Face>& leftTs, std::vector<Face>& rightTs)
{
	float minCost = std::numeric_limits<float>::max();
	Plane minCostPlane;
	Side minCostSide;
	...
	for (int i = 0; i < tsbb.size(); i++)
	{
		...
		for (int j = 0; j < splitCandidates.size(); j++)
		{
			...
			// Get the number of triangles to the left(tl), to the right(tr) and the number that of t's that lie in the plane(tp)
			std::vector<int> tl, tr, tp;
			classify(tsbb, splitCandidates[j], tl, tr, tp);
			// Get the cost
			// side will tell us to which side the triangles that lie on the plane should go in order to minimize the cost
...
bool KDTree::eventSahPartition(const std::vector<Face>& ts, BoundingBox nodebb, Plane& splitP, std::vector<Face>& leftTs, std::vector<Face>& rightTs)
{
	// Lets save the clipped triangles bounding box and their indices in the 'ts' vector here
	std::vector<std::pair<BoundingBox, int>> tsbb;
	// Clip all triangles and get the clipped triangles bounding box
...
/*
void KDTree::presortedGenEventsFromClipped(const std::vector<std::pair<Face*, Side*>>& tfs, const BoundingBox& nodebb, std::vector<PEvent>& events)
{
	// Lets save the clipped triangles bounding box and their indices in the 'ts' vector here
	std::vector<std::pair<BoundingBox, int>> tsbb;
	// Clip all triangles and get the clipped triangles bounding box
	...
	for (int dim = 0; dim < 3; dim++)
	{
		...
		for (int i = 0; i < tsbb.size(); i++)
		{
			...
			if (tsbb[i].first.min[dim] == tsbb[i].first.max[dim])
			{
				events.emplace_back(Plane(dim, tsbb[i].first.min[dim]), tfs[tsbb[i].second].first, tfs[tsbb[i].second].second, EVENT_TYPE_PLANAR);
			}
			else
			{
				events.emplace_back(Plane(dim, tsbb[i].first.min[dim]), tfs[tsbb[i].second].first, tfs[tsbb[i].second].second, EVENT_TYPE_START);
				events.emplace_back(Plane(dim, tsbb[i].first.max[dim]), tfs[tsbb[i].second].first, tfs[tsbb[i].second].second, EVENT_TYPE_END);
			}
		}
	}
}
