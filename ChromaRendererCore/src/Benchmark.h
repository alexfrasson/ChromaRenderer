...
public:
	...
	void startBenchmark(const unsigned int n, _Functor f)
	{
		runThread = std::thread([this, f, n](){ run(n, f); return 0; });
		//auto bound = std::bind(&Benchmark::run<_Functor>, this, std::placeholders::_1, std::placeholders::_2);
		//runThread = std::thread(bound, n, f);
	}
	template <typename _Functor, typename _Cmp>
	void startBenchmark(const unsigned int n, _Functor f, _Cmp isrunning)
	{
		runThread = std::thread([this, isrunning, f, n](){ run(n, f, isrunning); return 0; });
		//auto bound = std::bind(&Benchmark::run<_Functor>, this, std::placeholders::_1, std::placeholders::_2);
		//runThread = std::thread(bound, n, f);
	}
	void stopBenchmark();
...
private:
	template <typename _Functor>
	void run(const unsigned int& n, _Functor f)
	{
		stop = false;
		running = true;
		reset();
	...
	template <typename _Functor, typename _Cmp>
	void run(const unsigned int& n, _Functor f, _Cmp isrunning)
	{
		stop = false;
		running = true;
		reset();
