...
namespace RTUtils
{
	void trace(Scene &scene, Image &img, RendererSettings &settings, std::atomic<int>& pixelcount, Interval interval, bool& abort);
	...
	bool hitBoundingBoxSlab(const BoundingBox& bb, const Ray& r, const glm::vec3& invRayDir, const glm::bvec3& dirIsNeg, float& tmin, float& tmax);
	...
	bool closestIntersectionRayObjects(const Ray &ray, const std::vector<Object> &objects, const Face** hitTriangle, float &hitDistance, const bool boundingboxtest);
	bool intersectUntilRayObjects(const Ray &ray, const std::vector<Object> &objects, const glm::vec3 point, const bool boundingboxtest);
	...
	bool intersectRayTrianglesMollerTrumboreSIMD128(const Ray &r, const std::vector<Face>& f, const Face** hitTriangle, float &hitDistance);
	bool intersectRayObjectMollerTrumboreSIMD128(const Ray &r, const Object &object, const Face** hitTriangle, float &hitDistance);
	bool intersectRayObjectMollerTrumboreSIMD256(const Ray &r, const Object &object, const Face** hitTriangle, float &hitDistance);
}