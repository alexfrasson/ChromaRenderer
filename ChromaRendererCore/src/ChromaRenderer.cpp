	...
	for (int i = 0; i < widthDivs; i++)
	{
		...
		for (int j = 0; j < heightDivs; j++)
		{
			...
			case ChromaRenderer::RAYCAST:
				threadPool.enqueue(std::bind(&RayCasting::trace, std::ref(renderer), std::ref(scene), std::ref(image), std::ref(settings), interval, std::placeholders::_1));
				break;
			case ChromaRenderer::PATHTRACE:
				threadPool.enqueue(std::bind(&PathTracing::trace, std::ref(pathtracing), std::ref(scene), std::ref(image), interval, std::placeholders::_1));
				break;
			}
		}
	}
...
void ChromaRenderer::start()
{
	...
	case ChromaRenderer::PATHTRACE:
		...
		break;
	case ChromaRenderer::CUDAPATHTRACE:
		//threadPool.enqueue(std::bind(&CudaPathTracer::renderThread, std::ref(cudaPathTracer), std::ref(image), std::placeholders::_1));
		break;
	}
...
void ChromaRenderer::importScene(std::string filename, std::function<void()> onLoad)
{
	...
	std::cout << "    Faces         " << (sizeof(Face) * o.f.size()) / 1024 << "KB" << "   #" << o.f.size() << std::endl;
	std::cout << std::endl;

	std::cout << "Mesh   total size " << m.sizeInBytes() / 1024 << "KB" << std::endl;
	std::cout << "    Triangles     " << (sizeof(Triangle) * m.t.size()) / 1024 << "KB" << "   #" << m.t.size() <<  std::endl;
	std::cout << "    Vertices      " << (sizeof(glm::vec3) * m.v.size()) / 1024 << "KB" << "   #" << m.v.size() << std::endl;
	std::cout << "    Normals       " << (sizeof(glm::vec3) * m.n.size()) / 1024 << "KB" << "   #" << m.n.size() << std::endl;
	...
	//	static_cast<std::function<void(Mesh*)>>(std::bind(&ChromaRenderer::cbSceneLoadedm, std::ref(*this), std::placeholders::_1)))
	//	);
	...
	ModelImporter::importcbscene(filename, scene,
		static_cast<std::function<void()>>(std::bind(&ChromaRenderer::cbSceneLoadedScene, std::ref(*this), onLoad)));
}
...
void ChromaRenderer::cbSceneLoadedm(Mesh *m)
{
	...
	scene.clear();
	threadPool.enqueue(
		std::bind(static_cast<void(Scene::*)(Mesh*, std::function<void(void)>)>(&Scene::addMesh), std::ref(scene), m,
...
void ChromaRenderer::cbSceneLoaded(Object o)
{
	scene.clear();
	threadPool.enqueue(
		std::bind(static_cast<void(Scene::*)(Object, std::function<void(void)>)>(&Scene::addObject), std::ref(scene), o,
		static_cast<std::function<void(void)>>(std::bind(&ChromaRenderer::cbSceneProcessed, std::ref(*this))))
		);
	state = State::PROCESSINGSCENE;
}
