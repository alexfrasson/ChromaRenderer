...
void getTotalNumvberOfTrianglesAndVertices(const aiScene* scene, const aiNode* node, uint64_t* nTriangles, uint64_t* nVertices)
{
...
// 
void convertToMeshRecursive(Scene& s, const aiScene* scene, const aiNode* node, aiMatrix4x4 transform, Mesh* m, uint32_t& offset)
{
...
bool convert(const aiScene* aiscene, Scene& s)
{
	...
	if (aiscene->HasCameras())
	{
		...
		s.camera.height = s.camera.width / cam->mAspect;
