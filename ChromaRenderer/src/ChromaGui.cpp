	...
	if (ImGui::BeginMainMenuBar())
	{
		...
		if (ImGui::BeginMenu("File"))
		{
			if (ImGui::BeginMenu("Import"))
			{
				...
				if (ImGui::MenuItem("Scene...", nullptr, nullptr, !(cr->getState() == ChromaRenderer::State::LOADINGSCENE)))
				{
					path.clear();
	...
	struct funcs
	{
		static bool ShowDummyObject(Material& mat, int uid)
		{
			...
			if (node_open)
			{
				ImGui::PushID(0);
				ImGui::AlignTextToFramePadding();
				ImGui::TreeNodeEx("KD", ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen | ImGuiTreeNodeFlags_Bullet);
				...
				ImGui::PushID(1);
				ImGui::AlignTextToFramePadding();
				ImGui::TreeNodeEx("KE", ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen | ImGuiTreeNodeFlags_Bullet);
				...
				ImGui::PushID(2);
				ImGui::AlignTextToFramePadding();
				ImGui::TreeNodeEx("Transparency", ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen | ImGuiTreeNodeFlags_Bullet);
...
bool ChromaGui::SettingsWindow(ChromaRenderer* cr)
{
	...
	ImGui::Begin("Settings");
	{
		...
		if (!(cr->getState() == ChromaRenderer::State::LOADINGSCENE) && ImGui::Button(cr->isRunning() ? "Stop" : "Render"))
		{
			if (cr->isRunning())
				cr->stopRender();
			else
...
bool ChromaGui::ViewportWindow(ChromaRenderer* cr)
{
	...
	ImGui::Begin("Viewport");
	{
		...
		ImGui::SameLine();
		ImGui::ProgressBar(cr->cudaPathTracer.getProgress(), ImVec2(-1, 0), (std::to_string(cr->cudaPathTracer.finishedSamplesPerPixel) + std::string("/") + std::to_string(cr->cudaPathTracer.targetSamplesPerPixel)).c_str());

		//float windowWidth = ImGui::GetWindowWidth() - ImGui::GetStyle().WindowPadding.x * 2 - ImGui::GetStyle().ItemInnerSpacing.x;
		...
		ImGui::Image((ImTextureID)cr->image.textureID, ImVec2(width, height), ImVec2(0, 1), ImVec2(1, 0), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 200));
		...
		if (cr->isRunning() && ImGui::IsWindowHovered(ImGuiHoveredFlags_::ImGuiHoveredFlags_None))
		{
			if (ImGui::IsMouseDragging(1))
			{
				...
				{
					vec2 angle = vec2(ImGui::GetIO().MouseDelta.x, ImGui::GetIO().MouseDelta.y) * ImGui::GetIO().DeltaTime * lookSens;
					...
					cr->scene.camera.forward = normalize(rotate(cr->scene.camera.forward, angle.y, cr->scene.camera.right));
					cr->scene.camera.right = normalize(cross(cr->scene.camera.forward, vec3(0, 1, 0)));
					cr->scene.camera.up = -normalize(cross(cr->scene.camera.forward, cr->scene.camera.right));
