...
void Camera::lookAt(glm::vec3 target)
{
	...
	if (eye.x == target.x && eye.z == target.z && eye.y > target.y) { // camera looking vertically down
		right = glm::vec3(0, 0, 1);
		up = glm::vec3(1, 0, 0);
		forward = glm::vec3(0, 1, 0);	
	}
	
	if (eye.x == target.x && eye.z == target.z && eye.y < target.y) { // camera looking vertically up
...
void Camera::randomRayDirection(const int i, const int j, Ray &ray) const
{
	...
	ray.direction = glm::normalize((float)(i + rand_halton[0] - width*0.5f) * right + (float)(j + rand_halton[1] - height*0.5f) * v - d * forward);*/
	...
	ray.direction = glm::normalize((float)(i + random0 - width*0.5f) * right + (float)(j + random1 - height*0.5f) * up - d * forward);
...
void Camera::rayDirection(const int i, const int j, std::vector<Ray> &rays) const
{
	...
	for(float k = -0.5f; k < 0.49999f; k += step)
	{
		for(float l = -0.5f; l < 0.49999f; l += step)
		{
			...
			ray.direction = glm::normalize(ray.direction);
			rays.push_back(ray);*/
			rays.emplace_back(eye, glm::normalize((float)(i + l - width*0.5f) * right + (float)(j + k - height*0.5f) * up - d * forward));
		}
	}
}
...
{
	const int numDivs = std::sqrtf(nRays);	//Number of subcells = numDivs^2
	const float step = 1.0f / numDivs;
